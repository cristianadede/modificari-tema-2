...
void multime_pereche::citire() //functie(metoda) de citire
{
    ...
    cin>>n;
    while(typeid(int)!=typeid(n)){cout<<"nu"; cin>>n;}
    p=new pereche[n];
     for(int i=0;i<n;i++)
...
class Luna
{
    ...
    public:
    Luna();
    Luna(const Luna&);//const de copiere
    Luna(string);//const de initializare cu o valoare data
    virtual~Luna();//destructor
    Luna& operator=(const Luna &);
    friend ostream& operator<<(ostream&, const Luna&);
    friend istream& operator>>(istream&, Luna&);
    virtual void citire();
...
Luna::Luna(string x="ianuarie") //constructor de initializare
{
  luna=x;
}
Luna::Luna(const Luna&ob) //constructor de copiere
{
  luna=ob.luna;
}
Luna::~Luna()//destructor
{
    cout<<"Distrugere obiect";
}
Luna& Luna :: operator=(const Luna &ob) //suprascriere operator de atribuire
{
    if(this!=&ob)
    {
        luna=ob.luna;
...
ostream& operator<<(ostream& out, const Luna& ob){
...
istream& operator>>(istream& in, Luna& ob){
...
public:
    ...
    ZiLuna(int x,string y) : Luna(y)
    {
           zi=x;
           luna=y;
    }
    ZiLuna(const ZiLuna&ob) : Luna(ob)
    {
    ...
    ZiLuna& operator=(const ZiLuna &ob)
    {
        if(this!=&ob)
        {
            this->Luna::operator=(ob);
        }
    return*this;
    }
    ~ZiLuna(){cout<<"distrugere obiect";}
    friend ostream& operator<<(ostream&, const ZiLuna&);
    friend istream& operator>>(istream&, ZiLuna&);

};
ostream& operator<<(ostream& out, const ZiLuna& ob){
...
istream& operator>>(istream& in, ZiLuna& ob){
...
public:
    ...
    LunaAn(int x,string y) : Luna(y)
    {
           an=x;
           luna=y;
    }
    LunaAn(const LunaAn&ob) : Luna(ob)
    {
    ...
    LunaAn& operator=(const LunaAn &ob)
    {
        if(this!=&ob)
        {
            this->Luna::operator=(ob);
    ...
    ~LunaAn(){cout<<"distrugere obiect";}
    friend ostream& operator<<(ostream&, const LunaAn&);
    friend istream& operator>>(istream&, LunaAn&);

};
ostream& operator<<(ostream& out, const LunaAn& ob){
...
istream& operator>>(istream& in, LunaAn& ob){
...
class DataCompleta : public ZiLuna, public LunaAn
{
    public:
    DataCompleta():ZiLuna(),LunaAn()
    {
        luna="ianuarie"
        zi=1;
        an=2019;
    }
    DataCompleta(int x,int y, string z) : ZiLuna(x,z), LunaAn(y,z)
    {
           an=y;
           luna=z;
           zi=x;
    }
    DataCompleta(const LunaAn&ob) : ZiLuna(ob), LunaAn(ob)
    {
    ...
    DataCompleta& operator=(const DataCompleta &ob)
    {
        if(this!=&ob)
        {
            this->ZiLuna::operator=(ob);
            this->LunaAn::operator=(ob);
        }
    return*this;
    }
    ~DataCompleta(){cout<<"distrugere obiect";}
    friend ostream& operator<<(ostream&, const DataCompleta&);
    friend istream& operator>>(istream&, DataCompleta&);

};
ostream& operator<<(ostream& out, const DataCompleta& ob){
...
istream& operator>>(istream& in, DataCompleta& ob){
...
class ObiectActual : public DataCompleta, public pereche
{
    public:
    ObiectActual():DataCompleta(),pereche()
    {
        luna="ianuarie"
        zi=1;
        an=2019;
    }
    ObiectActual(int x,int y, string z,int a,int b) : DataCompleta(x,y,z),pereche(a,b)
    {
           an=y;
           luna=z;
           zi=x;
    ...
    ObiectActual(const ObiectActual&ob) : DataCompleta(ob),pereche(ob)
    {
    ...
    ObiectActual& operator=(const ObiectActual &ob)
    {
        if(this!=&ob)
        {
            this->DataCompleta::operator=(ob);
            this->pereche::operator=(ob);
        }
    return*this;
    }
    ~ObiectActual(){cout<<"distrugere obiect";}
    friend ostream& operator<<(ostream&, const ObiectActual&);
    friend istream& operator>>(istream&, ObiectActual&);

};
ostream& operator<<(ostream& out, const ObiectActual& ob){

    out<<ob.zi<<' '<<ob.luna<<' '<<ob.an<<ob.nr1<<' '<<ob.nr2;
 return out;

};

istream& operator>>(istream& in, ObiectActual& ob){

    in>>ob.zi>>ob.luna>>ob.an>>ob.nr1>>ob.nr2;
